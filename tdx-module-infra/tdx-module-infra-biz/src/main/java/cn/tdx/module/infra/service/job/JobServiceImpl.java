package cn.tdx.module.infra.service.job;
...
import cn.tdx.framework.quartz.core.scheduler.SchedulerManager;
import cn.tdx.framework.quartz.core.util.CronUtils;
import cn.tdx.module.infra.controller.admin.job.vo.job.JobPageReqVO;
import cn.tdx.module.infra.controller.admin.job.vo.job.JobSaveReqVO;
import cn.tdx.module.infra.dal.dataobject.job.JobDO;
import cn.tdx.module.infra.dal.mysql.job.JobMapper;
import cn.tdx.module.infra.enums.job.JobStatusEnum;
...
import static cn.tdx.framework.common.util.collection.CollectionUtils.containsAny;
...
public class JobServiceImpl implements JobService {
    ...
    public void updateJob(JobSaveReqVO updateReqVO) throws SchedulerException {
        ...
        // 3. 更新 Job 到 Quartz 中
        schedulerManager.updateJob(job.getId(), job.getHandlerName(), updateReqVO.getHandlerParam(), updateReqVO.getCronExpression(),
    ...
    public void updateJobStatus(Long id, Integer status) throws SchedulerException {
        ...
        if (JobStatusEnum.NORMAL.getStatus().equals(status)) { // 开启
            schedulerManager.resumeJob(job.getId(), job.getHandlerName());
        } else { // 暂停
            schedulerManager.pauseJob(job.getId(), job.getHandlerName());
        }
    }
    ...
    public void syncJob() throws SchedulerException {
        ...
        for (JobDO job : jobList) {
            // 2.1 先删除，再创建
            schedulerManager.deleteJob(job.getId(), job.getHandlerName());
            schedulerManager.addJob(job.getId(), job.getHandlerName(), job.getHandlerParam(), job.getCronExpression(),
    ...
    public void deleteJob(Long id) throws SchedulerException {
        ...
        // 删除 Job 到 Quartz 中
        schedulerManager.deleteJob(id, job.getHandlerName());
    }
