...
public abstract class FlowLongScheduler {
                ...
                if (ObjectUtils.isNotEmpty(flwTaskList)) {
                    for (FlwTask flwTask : flwTaskList) {
                        ...
                        if (null != flwTask.getRemindTime()) {
                            ...
                            taskService.updateTaskById(temp, this.getAutoFlowCreator(flwTask.getTenantId()));
                        }
                        /*
                         * 任务超时
                         */
                        if (null != flwTask.getExpireTime()) {
                            ...
                            if (TaskType.timer.eq(flwTask.getTaskType()) || TaskType.trigger.eq(flwTask.getTaskType())) {
                                if (!flowLongEngine.autoCompleteTask(flwTask.getId(), this.getAutoFlowCreator(flwTask.getTenantId()))) {
                            ...
                            if (termAuto != null && termAuto) {
                                ...
                                if (null == termMode) {
                                    // 执行超时
                                    context.getRuntimeService().timeout(flwTask.getInstanceId(), flwTask, this.getAutoFlowCreator(flwTask.getTenantId()));
                                    ...
                                    // 自动拒绝
                                    if (!flowLongEngine.autoRejectTask(flwTask, this.getAutoFlowCreator(flwTask.getTenantId()))) {
     ...
     */
    public FlowCreator getAutoFlowCreator(String tenantId) {
        TenantContextHolder.setTenantId(Long.valueOf(tenantId));
        return FlowCreator.of(tenantId, "1", "admin");
    }
}
